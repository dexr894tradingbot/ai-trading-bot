...
async def manage_active_trade(
    ...
    last_alert = LAST_PROGRESS_ALERT.get(trade["trade_id"], 0)
    for step in PROGRESS_ALERT_STEPS:
        if progress_pct >= step and step > last_alert:
            LAST_PROGRESS_ALERT[trade["trade_id"]] = step
            break
